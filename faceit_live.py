...
def main():
    ...
    # load models
    net = load_face_model()
    generator, kp_detector = demo.load_checkpoints(config_path=f'{first_order_path}config/vox-adv-256.yaml',
    ...
    while True:
        ...
        stream_v = cv2.copyMakeBorder(rgb, 0, 0, x_border if x_border >= 0 else 0, x_border if x_border >= 0 else 0,
                                      cv2.BORDER_CONSTANT)
...
def process_image(source_image, base, current, net, generator, kp_detector, relative):
    predictions = make_animation(source_image, [base, current], generator, kp_detector, relative=relative,
...
def find_face_cut(net, face):
    ...
    if not face_found:
        ...
        print(
            f'Cutting at: ({cut_x1, cut_y1}) ({cut_x2},{cut_y2} width:{(cut_x2 - cut_x1)} height: {(cut_y2 - cut_y1)})')
